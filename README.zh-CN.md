...
> ⚠️ **状态：早期开发中。** 桌面端可以回收判定放行的构建缓存，也可以逐项
> 强制删除经人工明确确认的 worktree。Codex 插件始终只读。

## 选择使用方式

| 方式 | 适合场景 | 是否会修改文件 |
|---|---|---|
| **桌面端** | 交互式扫描、回收缓存与删除单个 worktree | 仅在确认后修改 |
...
- **需要关注**：解释 worktree 被拦下的原因，并保留人工强制删除入口。
- **无需操作**：保留正在使用或受保护的 worktree。
...
通过全部安全门禁的构建缓存；源码与未提交改动不会受到影响。删除 worktree 是独立的
人工覆盖操作：必须输入分支名后才会执行 `git worktree remove --force`。该操作会删除
worktree 内的未提交或未跟踪内容，但会保留 Git branch ref。
